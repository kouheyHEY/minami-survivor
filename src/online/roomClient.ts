...
export class RoomError extends Error {
  constructor(message: string, readonly status: number, readonly room?: RoomView) {
    super(message)
  }
}

export const isWaiting = (room: RoomView): room is RoomView & { state: { status: 'waiting' } } =>
  room.state.status === 'waiting'

async function call(body: Record<string, unknown>): Promise<{ room: RoomView; token?: string }> {
  ...
  try {
    ...
    throw new RoomError('通信できませんでした。電波の良い場所でもう一度お試しください。', 0)
  }
  const data = await response.json().catch(() => ({}))
  if (!response.ok) throw new RoomError(data.error ?? '通信に失敗しました。', response.status, data.room)
  return data
}

export const roomApi = {
  create: (name: string) => call({ op: 'create', gameKey: GAME_KEY, name }),
  join: (code: string, name: string) => call({ op: 'join', gameKey: GAME_KEY, code, name }),
  get: (code: string, token: string) => call({ op: 'get', code, token }),
  act: (seat: SavedSeat, version: number, action: OnlineAction) =>
    call({ op: 'act', code: seat.code, token: seat.token, version, action }),
}
...
export function subscribeRoom(
  room: RoomView,
  handlers: { onUpdate: (version: number) => void; onConnection: (connected: boolean) => void },
) {
  ...
  import('@supabase/supabase-js')
    .then(({ createClient }) => {
      ...
      const channel = client
        ...
        .on('broadcast', { event: 'room-updated' }, ({ payload }) => handlers.onUpdate(Number(payload?.version)))
        .subscribe((status) => handlers.onConnection(status === 'SUBSCRIBED'))
      cleanup = () => {
...
export const seatStorage = {
  load(): SavedSeat | null {
    try {
      const saved = JSON.parse(localStorage.getItem(SEAT_STORAGE_KEY) ?? 'null')
